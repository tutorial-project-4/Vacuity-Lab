...
/// #7 점프 후 낙하(v0.2): 예고 시작 순간 플레이어 X를 착지점으로 고정(이후 보정 없음)
...
public partial class JumpSlamAction : Action
{
    ...
    Phase _phase;
    float _timer;
    float _landX;   // 예고 시작 순간 고정
    float _startX, _groundY;
    Rigidbody2D _rb;
    float _prevGravity;
    GameObject _hitbox;
    ...
    protected override Status OnStart()
    {
        ...
        Debug.Log($"[BossSlam] 예고 시작(착지점 x={_landX:F2}) @ {Time.time:F2}s"); // 예고 연출은 추후
    ...
    protected override Status OnUpdate()
    {
        ...
        switch (_phase)
        {
            ...
            case Phase.Jump:
                ...
                pos.y = _groundY + JumpHeight.Value * 4f * t * (1f - t); // 착지 y = 시작 y (보스는 바닥 전용)
    ...
    void BeginJump(GameObject self)
    {
        ...
        _groundY = self.transform.position.y;
    ...
    void Land()
    {
        ...
        Debug.Log($"[BossSlam] 착지(x={_landX:F2}) → 후딜 @ {Time.time:F2}s");
    }
}
